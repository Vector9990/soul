...
TOKEN = "7116404559:AAHTq7i9gH8GYLiZEQraPm3Vsu3sbbpAUVo"
FORWARD_CHANNEL_ID = -100215621934
CHANNEL_ID = -100215621934
error_channel_id = -100215621934
            ...
            if stored_key == key:
                ...
                for user_data in users.values():
                    if user_data.get('redeemed_key') == key:
                        return "This key has already been redeemed. Buy a new key from @Vectorbhai ."

                if not has_access(user_id):
                    users[str(user_id)] = {'redeemed_key': key, 'has_access': True}
                    save_users(users)

                    bot.send_message(user_id, "Buy a New code From @Vectorbhai")
    ...
    try:
        if not has_access(user_id):
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                       ...
                                       "*Contact the owner for assistance: @Vectorbhai.*", parse_mode='Markdown')
            return
